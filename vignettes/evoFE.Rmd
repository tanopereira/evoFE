...
| `power` | unary | Signed exponentiation: `sign(x) * |x|^p`, `p` ∈ {0.5, 1/3, 2, 3} |
...
| `divide` | binary | `x₁ / x₂` (0 where denominator is 0) |
| `normalized_difference` | binary | `(x₁ − x₂) / (|x₁| + |x₂| + 1e-6)` |
| `log_ratio` | binary | `log1p(|x₁|) − log1p(|x₂|)` |
...
### Group-by Aggregations (mixed cat × num → numeric)
...
| `groupby_quantile` | Per-group Q1 or Q3 (`q` ∈ {0.25, 0.75}) |
...
| `concat` | cat × cat → cat | Concatenates 2 or 3 categorical columns with underscore separator |
...
1. **Cross-Validation (`cv`)**: The default strategy. Evaluates fitness using
   K-fold cross-validation (`cv_folds` parameter).
2. **Train/Validation/Holdout Split (`split`)**: Useful for faster evaluation
   on larger datasets.  Configure with `evaluation_strategy = "split"` and
   `split_ratio` (e.g., `c(0.6, 0.2, 0.2)`).
   - The first two portions are the **Train** and **Validation** sets used to
     score candidates during search.
   - The third portion (if provided) is the **Holdout** set. It is evaluated
     **only once** on the final best individual to avoid data snooping.

You can also supply your own split assignments:
...
| `evaluator` | `"lightgbm"` | ML backend: `"lightgbm"`, `"xgboost"`, `"catboost"`, `"lm"`, `"keras3"`, or a registered custom name |
...
| `metric` | `"default"` | Optimisation metric |
...
| `complexity_penalty` | 0 | Subtract `penalty × n_genes` from raw fitness to favour shorter recipes |
...
| `raw_toggle_prob` | 0.15 | Probability of toggling raw feature(s) in the active mask during mutation |
| `recalculate_mask_prob` | 0.05 | Probability of redrawing the entire active mask from importances |
| `mask_temp_factor` | 0.5 | Temperature for importance-guided mask sampling (higher = more uniform) |
...
By default, a recipe with 10 genes that scores 0.821 beats a 1-gene recipe
that scores 0.820 — even though the longer recipe is more likely to overfit.
The `complexity_penalty` parameter adds parsimony pressure:

$$\text{adjusted fitness} = \text{raw fitness} - \alpha \times n_{\text{genes}}$$

A value of `0.001` is a good starting point:

```r
recipe <- evolve_features(
  data = df, target_col = "y", task = "classification",
  complexity_penalty = 0.001   # subtract 0.001 per gene
)
```

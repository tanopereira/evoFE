...
## Bayesian Hyperparameter Tuning: Zero-Boilerplate "One-Liners"

Hyperparameters and feature engineering interact strongly—certain transformations perform best under specific tree depths or learning rates. Rather than tuning models after the fact, evoFE supports **simultaneous feature evolution and Bayesian hyperparameter optimization** powered by `mlr3mbo`, `paradox`, and `bbotk`.

### Built-in One-Liner Tuners

evoFE ships with pre-configured Bayesian tuners registered at package load. You can activate full Bayesian hyperparameter tuning across every generation with a **single parameter change**:

```r
# One-liner LightGBM Bayesian hyperparameter tuning
recipe_lgb <- evolve_features(
  data        = df,
  target_col  = "am",
  task        = "classification",
  evaluator   = "lightgbm_mbo", # Automatically tunes lr, num_leaves, max_depth, feature_fraction
  generations = 5,
  pop_size    = 6,
  mbo_iters   = 5
)

# One-liner XGBoost Bayesian hyperparameter tuning
recipe_xgb <- evolve_features(
  data        = df,
  target_col  = "am",
  task        = "classification",
  evaluator   = "xgboost_mbo",  # Automatically tunes eta, max_depth, min_child_weight, subsample
  generations = 5,
  pop_size    = 6,
  mbo_iters   = 5
)
...
### One-Liner Custom Tuner Registration with `make_tunable()`

You can also wrap *any* registered evaluator (including custom models) in a Bayesian optimization loop in a single line using `make_tunable()`:

```r
# 1. Define custom parameter search space
custom_space <- list(
...
# 2. One-liner: turn standard xgboost into a Bayesian tuned evaluator
make_tunable("xgboost", param_ranges = custom_space, tuner_name = "xgboost_custom_tuned")

# 3. Use it immediately in evolution
recipe <- evolve_features(
  data = df, target_col = "am", task = "classification",
  evaluator = "xgboost_custom_tuned",
  mbo_iters = 5,
  mbo_init_design = 8
)
```

...
evoFE ships with **39 built-in transformers** that the genetic algorithm can
...
| `groupby_median` | Median within each group (robust to outliers) |
| `groupby_quantile` | Q1 or Q3 within each group (`q` ∈ {0.25, 0.75}) |
...
| `woe_encode` | cat → num | Weight of Evidence: `ln(P(event\|cat) / P(non-event\|cat))` with Laplace smoothing; binary classification only |
...
| `genie_centroid_dist` | numeric | Distance to Genie cluster centroid |
| `lumbermark` | categorical | Lumbermark hierarchical clustering |
| `lumbermark_centroid_dist` | numeric | Distance to Lumbermark cluster centroid |
...
| `early_stopping_generations` | 3 | Stop if no improvement for *n* generations |
...
| `threads` | 2 | Parallelism for model training |
| `metric` | `"default"` | Optimization metric: `"default"`, `"auc"`, `"f1"`, `"mae"`, or a custom function |
| `allowed_transformers` | `"all"` | Transformer pool: `"all"`, `"basic"`, `"robust"`, `"clustering"`, or a character vector of names |
| `complexity_penalty` | `0` | Per-gene fitness penalty — subtracts `penalty × n_genes` from raw fitness to favour shorter recipes |
...
- **Use `set.seed()`** before calling `evolve_features()` for reproducible experiments and benchmarking.
- **`crossover_type = "union"`** tends to produce larger recipes (more features).
  `"random"` keeps recipes leaner.
- **Use `complexity_penalty`** on small datasets (e.g. `0.001`) to discourage
  long recipes and reduce overfitting.
- **Use `allowed_transformers = "robust"`** when you want the search restricted
  to statistically stable, outlier-resistant transformers without the heavier
  clustering or manifold-learning ops.

## Advanced Options

### Transformer Presets

The `allowed_transformers` parameter controls which transformers the genetic
algorithm can select. Beside a custom character vector of transformer names,
four named presets are available:

| Preset | Transformers | When to use |
|:---|:---:|:---|
| `"all"` | 39 | Default — full search space |
...
| `"clustering"` | 10 | Manifold & graph features only (UMAP, Genie, MST…) |

```r
# Restrict to the robust preset
recipe <- evolve_features(
  data = df, target_col = "y", task = "classification",
  allowed_transformers = "robust"
)
```
...
The `complexity_penalty` parameter adds a parsimony pressure:

$$\text{adjusted fitness} = \text{raw fitness} - \alpha \times n_{\text{genes}}$$

A value of `0.001` is a good starting point for most datasets:
...
Set to `0` (default) to disable. Has no effect on the baseline individual
(0 genes) or individuals whose raw fitness is already `-Inf`.
...
`0.95`). This prevents the search from wasting evaluations on near-duplicate
columns.

The threshold is tunable via an option:
...
`plot(recipe)` (or `plot(recipe, type = "fitness")`) produces a
generation-by-generation fitness curve with:

- A **shaded region** showing improvement over the generation-1 baseline
- A **dashed reference line** at the generation-1 fitness
- A **gold marker** on the best generation
- A subtitle reporting total generations, best fitness, and total gain

```r
plot(recipe, type = "fitness")
plot(recipe, type = "importance")  # top feature importances
```

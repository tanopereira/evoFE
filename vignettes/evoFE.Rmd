...
### Built-in One-Liner Tuner: `lightgbm_mbo`

evoFE ships with a pre-configured Bayesian tuner for LightGBM registered at package load. You can activate full Bayesian hyperparameter tuning across every generation with a **single parameter change**:
...
### One-Liner Custom Tuner Registration: `make_tunable()`

For XGBoost or any other registered ML backend, `make_tunable()` wraps the base evaluator in a Bayesian optimization loop in a single line:
...
# 2. One-liner: register a Bayesian tuned evaluator (e.g. 'xgboost_mbo')
make_tunable("xgboost", param_ranges = xgb_space, tuner_name = "xgboost_mbo")

...
test_that("CatBoost evaluator checks for package availability", {
  ...
  if (system.file(package = "catboost") == "") {
    ...
    res <- train_model(x_train, y_train,
      x_val = x_train, task = "classification", evaluator = "catboost",
...
test_that("lightgbm_mbo checks for package availability", {
  ...
  if (!requireNamespace("mlr3mbo", quietly = TRUE) ||
    ...
    res <- train_model(x_train, y_train,
      x_val = x_train, task = "classification",
    ...
    res_seeded <- train_model(x_train, y_train,
      x_val = x_train, task = "classification",
...
test_that("make_tunable works correctly", {
  ...
  res <- train_model(
    x_train, y_train,
    x_val = x_val, y_val = y_val, task = "regression",
...
test_that("compute_complexity_penalty returns 0 when disabled", {
  expect_equal(compute_complexity_penalty(n_genes = 3, n_samples = 1000, complexity_penalty = 0), 0)
  expect_equal(compute_complexity_penalty(n_genes = 3, n_samples = 1000, complexity_penalty = 1, complexity_mode = "none"), 0)
  expect_equal(compute_complexity_penalty(n_genes = 0, n_samples = 1000, complexity_penalty = 1, complexity_mode = "bic_dynamic"), 0)
})
...
test_that("compute_complexity_penalty dynamically scales convergence and enforces 20% default safety floor", {
  ...
  # Classification: baseline = 0.70, ideal = 1.00
  # At baseline (Gen 0): factor = 1.00
  ...
  expect_equal(pen_start, bic_base)

  # Halfway (best = 0.85): factor = 0.50
...
test_that("compute_complexity_penalty handles regression (negative loss) metrics correctly", {
  ...
  # Regression (-RMSE): baseline = -100, ideal = 0.0
  # At baseline (best = -100): factor = (0 - (-100)) / (0 - (-100)) = 1.0
  pen_reg_start <- compute_complexity_penalty(
    n_genes = 1, n_samples = n, running_best_fitness = -100, baseline_fitness = -100,
    task = "regression", complexity_penalty = 1.0, complexity_mode = "bic_dynamic"
  )
  expect_equal(pen_reg_start, bic_base)

  # Halfway (best = -50): factor = (0 - (-50)) / 100 = 0.50
  pen_reg_mid <- compute_complexity_penalty(
    n_genes = 1, n_samples = n, running_best_fitness = -50, baseline_fitness = -100,
    task = "regression", complexity_penalty = 1.0, complexity_mode = "bic_dynamic"
  )
...
test_that("evolve_features runs successfully with complexity_mode = bic_dynamic and bic", {
  ...
  expect_s3_class(res_dyn, "evo_recipe")
  expect_true(is.numeric(res_dyn$best_individual$fitness))

  # Test static BIC
  res_bic <- evolve_features(
    data = df,
    target_col = "target",
    task = "classification",
    generations = 2,
  ...
  expect_s3_class(res_bic, "evo_recipe")
...
test_that("evaluate_fitness sets raw_fitness, penalty, and fitness correctly", {
  ...
  gene1 <- create_gene("log", "x1")
  ind <- create_individual(genes = list(gene1), numeric_cols = c("x1", "x2"))

  # With complexity penalty
  ind_eval <- evaluate_fitness(
    ind, df, "y", task = "classification", cv_folds = 2,
    complexity_penalty = 1.0, complexity_mode = "bic"
  )
  ...
  expect_equal(ind_no_pen$penalty, 0)
  expect_equal(ind_no_pen$fitness, ind_no_pen$raw_fitness)
})

test_that("print.evo_recipe and summary.evo_recipe output raw_fitness and penalty cleanly", {
  set.seed(42)
  df <- data.frame(
    x1 = rnorm(30),
    target = sample(0:1, 30, replace = TRUE)
  ...
  recipe <- evolve_features(
    data = df, target_col = "target", task = "classification",
    generations = 1, pop_size = 2, cv_folds = 2,
    complexity_penalty = 1.0, complexity_mode = "bic", verbose = FALSE
  )

  # Check print output contains validation score
  out <- utils::capture.output(print(recipe))
  expect_true(any(grepl("Validation Score:", out) | grepl("Best Fitness:", out)))

  # Check summary output contains metric score and penalty
  s_out <- utils::capture.output(print(summary(recipe)))
